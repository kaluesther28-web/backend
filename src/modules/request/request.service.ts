...
//     // Check user has no active request already
...
//     // Only the user or assigned provider can view the request
...
//   static async getUserRequests(userId: ObjectId | string) {
...
//   static async getProviderRequests(providerId: ObjectId | string) {
...
//     if (request.serviceType !== provider.serviceType) {
//       throw ApiError.badRequest(
//         `This request requires a ${request.serviceType}, but you are a ${provider.serviceType}`
//       );
//     }

//     request.providerId       = provider._id as ObjectId;
...
//   // PROVIDER: Mark as en route (on the way to customer)
//   static async markEnRoute(
//     requestId: string,
//     providerUserId: ObjectId | string
//   ) {
//     const provider = await Provider.findOne({ userId: providerUserId });
...
//       throw ApiError.badRequest(
//         `Cannot mark en route — current status is: ${request.status}`
//       );
//     }
...
//   // PROVIDER: Mark as arrived at customer location
...
//       throw ApiError.badRequest(
//         `Cannot mark arrived — current status is: ${request.status}`
//       );
//     }
...
//     return ApiSuccess.ok("You have arrived at the customer location", {
//       request,
//     });
//   }

//   // PROVIDER: Mark job as completed with final cost
...
//       throw ApiError.badRequest(
//         `Cannot complete — current status is: ${request.status}`
//       );
//     }
...
//       $inc: {
//         totalJobs:       1,
//         "earnings.total": providerEarning,
//       },
//       status: "online",
//     });
...
//   // USER: Cancel a pending or accepted request
...
export class RequestService {
  // USER: Create a new service request
  static async createRequest(userId: ObjectId | string, data: CreateRequestDTO) {
    const { serviceType, description, longitude, latitude, address } = data;
  ...
  // USER / PROVIDER: Get a single request by ID
  static async getRequestById(requestId: string, userId: ObjectId | string) {
    const request = await Request.findById(requestId)
      .populate("userId",     "firstName lastName phoneNumber")
      .populate("providerId", "businessName serviceType rating location");
  ...
  // PROVIDER: Get all requests assigned to them
  static async getProviderRequests(providerUserId: ObjectId | string) {
    const provider = await Provider.findOne({ userId: providerUserId });
    if (!provider) throw ApiError.notFound("Provider profile not found");
  ...
  static async getNearbyRequests(providerUserId: ObjectId | string) {
    const provider = await Provider.findOne({ userId: providerUserId });
    if (!provider) throw ApiError.notFound("Provider profile not found");
  ...
  // PROVIDER: Accept a pending request
  static async acceptRequest(requestId: string, providerUserId: ObjectId | string) {
    const provider = await Provider.findOne({ userId: providerUserId });
    if (!provider) throw ApiError.notFound("Provider profile not found");
  ...
  // PROVIDER: Decline a pending request
  static async declineRequest(requestId: string, providerUserId: ObjectId | string) {
    const request = await Request.findById(requestId);
    if (!request) throw ApiError.notFound("Request not found");
  ...
  static async markEnRoute(requestId: string, providerUserId: ObjectId | string) {
    const provider = await Provider.findOne({ userId: providerUserId });
    if (!provider) throw ApiError.notFound("Provider profile not found");
  ...
  static async markArrived(requestId: string, providerUserId: ObjectId | string) {
    const provider = await Provider.findOne({ userId: providerUserId });
    if (!provider) throw ApiError.notFound("Provider profile not found");
